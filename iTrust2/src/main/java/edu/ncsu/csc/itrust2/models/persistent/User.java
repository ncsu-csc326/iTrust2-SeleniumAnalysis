...
public class User extends DomainObject<User> implements Serializable {
    ...
    public void delete () {
        try {
            @SuppressWarnings ( "unchecked" )
            final List<PasswordResetToken> list =
                    ((List<PasswordResetToken>) DomainObject.getAll( PasswordResetToken.class ))
                    .stream()
                    .filter( x -> x.getUser().equals( this ) ).collect( Collectors.toList() );
